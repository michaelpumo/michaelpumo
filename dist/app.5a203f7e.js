...
function confetti() {
  ...
  var options = {
    items: function items() {
      return window.innerWidth * window.innerHeight / (window.innerWidth / 0.06);
    },
    width: 6,
    height: 60,
    colours: _settings.default.colors
  };
  var shapes = [];
  ...
  function drawShape(shape) {
    ...
    ctx.rotate((0, _helpers.degToRad)(shape.rotate));
  ...
  function setShape(shape, index) {
    ...
    if (shape.y > canvas.height) {
      ...
      shape.x = (0, _helpers.randNumber)(0, canvas.width);
    } else {
      shape.y = shape.y += ySpeed; // if (index % 2) {
  ...
  function createShapes() {
    for (var i = 0; i < options.items(); i += 1) {
      var shapeEl = {
        width: options.width,
        height: options.height,
        x: (0, _helpers.randNumber)(-Math.abs(options.width), window.innerWidth),
        y: (0, _helpers.randNumber)(-Math.abs(options.height), window.innerHeight),
        colour: color ? options.colours.find(function (item) {
          return item.name === color;
        }).value : options.colours[(0, _helpers.randNumber)(0, options.colours.length - 1)].value,
...
var app = {
  init: function init() {
    ...
    (0, _patterns.default)('#pattern-cover', false, true);
    (0, _patterns.default)('#pattern-reference', 'cyan');
    (0, _patterns.default)('#pattern-availability', 'yellow');
    (0, _patterns.default)('#pattern-resume', 'purple');
  }
};
