...
export default {
  ...
  computed: {
    messages() {
      ...
      const formatted = Object.keys(validation.$params)
        .filter((item) => Object.prototype.hasOwnProperty.call(validation, item) && !validation[item])
  ...
  methods: {
    getMessage(name, item) {
      ...
      if ((item) instanceof Object && !Object.prototype.hasOwnProperty.call(item, 'type')) {
        item.type = name
      }
